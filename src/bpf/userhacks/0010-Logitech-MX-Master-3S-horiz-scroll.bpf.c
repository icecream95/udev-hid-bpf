...
#define PID_MX_MASTER_3S_BLUETOOTH	0xB028

HID_BPF_CONFIG(
	HID_DEVICE(BUS_USB, HID_GROUP_GENERIC, VID_LOGITECH, PID_BOLT_RECEIVER);
);

#define MASTER3S_REPORT_DESCRIPTOR_LENGTH 54
...
int BPF_PROG(master_3s_fix_event, struct hid_bpf_ctx *hid_ctx)
{
	...
	struct master_3s_hid_report *data = (struct master_3s_hid_report*)hid_bpf_get_data(hid_ctx, 0, sizeof(*data));
...
{
	ctx->retval = ctx->rdesc_size != MASTER3S_REPORT_DESCRIPTOR_LENGTH;
	if (ctx->retval)
		ctx->retval = -EINVAL;
